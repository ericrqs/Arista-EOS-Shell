...
class AristaEOSAutoload(CiscoGenericSNMPAutoload):
    ...
    def _get_ip_interface_details(self, resource_obj, port_index):
        ...
        interface_details = {}
        self.logger.info('_get_ip_interface_details')
        if self.ip_v4_table and len(self.ip_v4_table) > 1:
            self.logger.info('table ok')
            for key, value in self.ip_v4_table.iteritems():
                self.logger.info('ip table item %s = %s' % (str(key), str(value)))
                if 'ipAdEntIfIndex' in value and int(value['ipAdEntIfIndex']) == port_index:
                    self.logger.info('got port index %d' % port_index)
